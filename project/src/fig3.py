...
def panel_c(ax, A):
    ...
    def sc(d, color, marker, label, s=16, minpers=0.008):
        if len(d) == 0:
            return
        ...
        ax.scatter(birth[keep], death[keep], s=s,
                   ...
                   linewidth=0.5, marker=marker, label=label, rasterized=True)
    ...
    sc(dP0, "#1f7a7a", "o", "present H$_0$")
    sc(dF0, "#b2182b", "o", "future H$_0$")
    sc(dP1, "#5fae57", "^", "present H$_1$", s=24)
    sc(dF1, "#8c2d62", "^", "future H$_1$", s=24)
    ...
    ax.legend(loc="lower right", fontsize=6.2, ncol=2, columnspacing=0.7)
...
def panel_d(ax, A):
    mask = A["mask"]
    # recompute landscapes from stored diagrams
    ...
    for i in range(LP.shape[0]):
        ax.plot(xs, LP[i], color="#1f7a7a", lw=1.1, alpha=0.85 - i * 0.15)
        ax.plot(xs, LF[i], color="#b2182b", lw=1.1, alpha=0.85 - i * 0.15, ls="-")
    ax.fill_between(xs, LP[0], color="#1f7a7a", alpha=0.10)
    ax.fill_between(xs, LF[0], color="#b2182b", alpha=0.10)
    ax.plot([], [], color="#1f7a7a", label="present")
    ax.plot([], [], color="#b2182b", label="future")
    ...
    # fragmentation index annotation
